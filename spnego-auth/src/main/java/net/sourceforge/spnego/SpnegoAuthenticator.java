...
public final class SpnegoAuthenticator {
    ...
    private transient boolean promptIfNtlm;

    /** Login Context module name for client auth. */
    private transient String clientModuleName;
    ...
    public SpnegoAuthenticator() throws Exception {
        String user = System.getProperty(Constants.PREAUTH_USERNAME);
        String passwd = System.getProperty(Constants.PREAUTH_PASSWORD);
        String serverLoginModule = System.getProperty(Constants.SERVER_MODULE, "spnego-server");

        if (user == null && passwd == null) {
            this.loginContext = new LoginContext(serverLoginModule);
        } else {
            final CallbackHandler handler = SpnegoProvider.getUsernamePasswordHandler(user, passwd);
    ...
    public SpnegoPrincipal authenticate(final HttpServletRequest req
        , final HttpServletResponse resp) throws GSSException
        , IOException {
    ...
    private SpnegoPrincipal doSpnegoAuth(
        final SpnegoAuthScheme scheme, final HttpServletResponse resp)
